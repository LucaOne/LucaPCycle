...
def predict_seq_level_regression(
    ...
    for idx, info in enumerate(batch_info):
        if args.input_mode == "pair":
            cur_res = [info[0], info[1], info[2], info[3], float(probs[idx][0]), float(probs[idx][0])]
            if len(info) > 4:
                cur_res += info[4:]
        else:
            cur_res = [info[0], info[1], float(probs[idx][0]), float(probs[idx][0])]
            if len(info) > 2:
                cur_res += info[2:]
        res.append(cur_res)
    return res
...
def run(
    ...
    lucapcycle_args.vector_dirpath = lucapcycle_args.emb_dir if lucapcycle_args.emb_dir and (os.path.exists(lucapcycle_args.emb_dir) or "#" in lucapcycle_args.emb_dir) else None
    lucapcycle_args.matrix_dirpath = lucapcycle_args.emb_dir if lucapcycle_args.emb_dir and (os.path.exists(lucapcycle_args.emb_dir) or "#" in lucapcycle_args.emb_dir) else None
